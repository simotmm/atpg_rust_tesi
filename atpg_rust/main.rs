...
mod util;
use util::{min, max, print_fault_list, print_pattern_list, print_simulation_results, get_nth_arg_to_int, get_nth_arg_to_string, get_nth_arg_to_bool, get_filename_from_int, print_sat_solution, sat_solutions_to_input_patterns, press_any_key, save_patterns_to_file, save_patterns_to_test_inputs};
...
const PARALLEL:           bool = false; // se true, simula i pattern in parallelo su più thread (sia per random che per sat), altrimenti in sequenziale (utile per debug e confronto con risultati sequenziali)
...
pub const SAVE_UNDETECTED_TO_FILE: bool = false; // se true, salva la lista dei fault non rilevati in un file di testo (nella stessa cartella del benchmark, con nome derivato dal benchmark)
...
fn main() {
    ...
    if !preconfigured {
        ...
        crate::options::set_options(crate::options::RunOptions { enable_sat, optimize: minimizer.is_some(), minimizer: minimizer.clone(), sat_backend: sat_backend_choice.clone() });
    }
    ...
    let n_patterns = if N_PATTERNS > 0 { N_PATTERNS } else { std::cmp::max(1, n_faults/decrease_factor) as usize};
    ...
    if run_random_phase {
        ...
        if PRINT { for pattern in &ppsfp_patterns { pattern.print(); } }
